...
- 以图 12-27 为例，解释RPF的原理:
![12.27](../pics/12.27.png) 
- 在这个拓扑中，我们有三个路由器（R1、R2和R3），它们都直接相连。我们将使用组播IP地址224.1.1.1向网络中的所有设备发送组播流量。
- 以下是R1、R2和R3的配置：
```shell
interface GigabitEthernet0/0
 ...
 ip verify multicast source reachable-via any allow-default
!
interface GigabitEthernet0/1
 ...
 ip verify multicast source reachable-via any allow-default
```
- 在这个配置中，我们启用了RPF检查，以确保只有从正确的接口接收到的组播流量才被路由器转发。例如，当R1收到来自R2的组播数据包时，它将检查该数据包的源地址是否通过任何接口可达。如果是，R1将接受该数据包并转发到其所有接口。但是，如果该数据包的源地址不能通过任何接口到达，R1将丢弃该数据包。同样，当R2收到来自R1的组播数据包时，它将执行同样的RPF检查。如果该数据包的源地址不能通过任何接口到达，R2将丢弃该数据包。使用RPF检查有助于确保组播流量只在正确的接口上传输，并防止不正确的转发可能导致的网络问题。
