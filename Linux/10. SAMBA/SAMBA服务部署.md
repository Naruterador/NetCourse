...
-  使用cifs协议、可跨平台、可实现文件系统挂载、可实现服务器端修改文件。
...
- 进程作用：smbd进程的作用是处理到来的SMB数据包，为使用该数据包的资源与Unix进行协商
...
| ------------------------------ | --------------------------------------------- |
| [Share]                        | 定义共享资源的名称                             |
| comment = smb warehouse        | 设置共享资源的注释                             |
...
![10.2.13](./pics/10.2.13.png)

图10.2.13
</center>

## 10.3 Samba共享服务身份验证 
### 10.3.1 配置Samba共享服务身份验证 
...
- 某公司需要创建一个公共存储空间名称为share，要求如下：
  - 存储路径为/opt/abc。
  - 要求公司员工都能对存储空间进行读写。
  - 公司员工在存储空间中新建的文件权限为644，新建的目录权限为755。
  - 创建smbclient公司员工帐户进行测试访问。
...
| browseable      | 开启目录访问             |
| writeable       | 允许目录写入功能         |
| create mask     | smb用户新建文件权限为644 |
| directory mask  | smb用户新建子目录权限为755 |
...
- 配置一台Samba服务器要求如下：
  - 用户身份模式为user,采用tdbsam验证机制。
  - 创建三个账号作为Samba登陆帐户：user1、user2、user3,密码和用户名相同。
  - 建立Samba共享目录/opt/sharesmb，共享名为smbdir。
  - 要求user1、user2和user3用户都具有上传权限。
...
- 创建user1,user2,user3用户，使用"smbpasswd -a"指令将所有用户都转换为Samba用户。操作如图10.3.11所示。

<center>

![10.3.11](./pics/10.3.11.png)
...
| valid users = user1,... | 允许指定用户访问该共享资源                |
